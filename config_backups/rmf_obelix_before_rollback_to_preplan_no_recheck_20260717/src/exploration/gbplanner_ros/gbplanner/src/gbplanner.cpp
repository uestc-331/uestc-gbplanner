...
void Gbplanner::initializeAttributes() {
  ...
  planner_validate_path_nh_.setCallbackQueue(&planner_validate_path_queue_);
  ros::AdvertiseServiceOptions validate_path_options =
      ros::AdvertiseServiceOptions::create<planner_msgs::planner_validate_path>(
          "gbplanner/validate_path",
          boost::bind(&Gbplanner::validatePathServiceCallback, this, _1, _2),
          ros::VoidPtr(), &planner_validate_path_queue_);
  planner_validate_path_service_ =
      planner_validate_path_nh_.advertiseService(validate_path_options);
  planner_validate_path_spinner_.reset(
      new ros::AsyncSpinner(1, &planner_validate_path_queue_));
  planner_validate_path_spinner_->start();
  ROS_WARN("[GBPLANNER][PATH_RECHECK] dedicated callback queue started");
...
bool Gbplanner::validatePathServiceCallback(
  ...
  res.valid = rrg_->validatePathOnCurrentMap(
      req.path, req.stop_at_unknown_voxel, collision_free, clearance_safe,
      res.min_clearance, res.clearance_cost, res.reason);
  res.collision_free = collision_free;
  res.clearance_safe = clearance_safe;
  ROS_WARN("[GBPLANNER][PATH_RECHECK] compute_time=%.4fs path_size=%zu "
           "valid=%d reason=%s",
           (ros::WallTime::now() - start).toSec(), req.path.size(), res.valid,
           res.reason.c_str());
  return true;
}
