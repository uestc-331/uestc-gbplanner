...
PlannerControlInterface::PlannerControlInterface(
  ...
  start_planner_sub_ = nh_.subscribe(
      ...
      &PlannerControlInterface::startPlannerTopicCallback, this);
  ...
  homing_sub_ = nh_.subscribe("planner_control_interface/std_srvs/homing_trigger",
                              1, &PlannerControlInterface::homingTopicCallback,
                              this);
  go_to_waypoint_sub_ =
      nh_.subscribe("planner_control_interface/std_srvs/go_to_waypoint", 1,
                    &PlannerControlInterface::goToWaypointTopicCallback, this);
  initialization_sub_ = nh_.subscribe(
      "pci_initialization_trigger", 1,
      &PlannerControlInterface::initializationTopicCallback, this);
  ...
  global_planner_sub_ = nh_.subscribe("pci_global", 1,
                                      &PlannerControlInterface::globalPlannerTopicCallback,
                                      this);
  ...
  stop_planner_sub_ = nh_.subscribe("planner_control_interface/std_srvs/stop", 1,
                                    &PlannerControlInterface::stopPlannerTopicCallback,
                                    this);
...
void PlannerControlInterface::startPlannerTopicCallback(
    const std_msgs::Empty& msg) {
  std_srvs::Trigger::Request req;
  std_srvs::Trigger::Response res;
  stdSrvsAutomaticPlanningCallback(req, res);
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Start planner topic trigger failed.");
  }
}

void PlannerControlInterface::stopPlannerTopicCallback(
    const std_msgs::Empty& msg) {
  std_srvs::Trigger::Request req;
  std_srvs::Trigger::Response res;
  stdSrvStopPlannerCallback(req, res);
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Stop planner topic trigger failed.");
  }
}

void PlannerControlInterface::homingTopicCallback(const std_msgs::Empty& msg) {
  std_srvs::Trigger::Request req;
  std_srvs::Trigger::Response res;
  stdSrvHomingCallback(req, res);
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Homing topic trigger failed.");
  }
}

void PlannerControlInterface::initializationTopicCallback(
    const std_msgs::Empty& msg) {
  planner_msgs::pci_initialization::Request req;
  planner_msgs::pci_initialization::Response res;
  ...
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Initialization topic trigger failed.");
  }
}

void PlannerControlInterface::goToWaypointTopicCallback(
    const std_msgs::Empty& msg) {
  std_srvs::Trigger::Request req;
  std_srvs::Trigger::Response res;
  stdSrvGoToWaypointCallback(req, res);
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Go-to-waypoint topic trigger failed.");
  }
}

void PlannerControlInterface::globalPlannerTopicCallback(
  ...
  req.vel_max = 0.0;
  req.id = msg.data;
  req.not_check_frontier = false;
  req.ignore_time = false;

  globalPlannerCallback(req, res);
  if (!res.success) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN,
                  "[PCI] Global planner topic trigger failed.");
  }
}
