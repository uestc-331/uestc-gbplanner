  ...
  for (int i = 0; i < num_leaf_vertices; ++i) {
    ...
    if (path_size > 1) {
      ...
      path_diag.short_frontier_step =
          ...
          path_passes_direction && path_passes_clearance;
  ...
  if (!path_diagnostics.empty()) {
    ...
    for (int i = 0; i < path_log_count; ++i) {
      ...
      ROS_WARN(
          ...
          "reverse_dot=%.3f clearance=%.3f cost=%.3f "
          "pass=[G:%d L:%d U:%d Dir:%d Clr:%d ALL:%d] "
          "pos=[%.2f %.2f %.2f]",
          ...
          diag.reverse_dot, diag.min_clearance, diag.clearance_cost,
          diag.pass_gain, diag.pass_length, diag.pass_density,
          diag.pass_direction, diag.pass_clearance, diag.pass_all,
