...
pci_exec_stuck_count="$(
  grep -E '\[PCI\]\[EXEC\] wp=[0-9]+/[0-9]+ dist=' "$LOG_FILE" \
    | sed -E 's/.*wp=([0-9]+\/[0-9]+) dist=([0-9.]+) thr=([0-9.]+).*/\1 \2 \3/' \
    | awk '
      NF == 3 && ($2 + 0) > ($3 + 0) {
        key = $1 " " $2 " " $3
        count[key]++
...
echo "pci_wp_stuck:   ${pci_exec_stuck_count:-0}"
...
if [[ "${pci_exec_stuck_count:-0}" -gt 0 ]]; then
  echo "warn: PCI appears stuck waiting for waypoint completion" >&2
  exit 1
fi
