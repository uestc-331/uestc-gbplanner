...
class Rrg {
 ...
 private:
  ...
  // History path tracking to prevent oscillation
  std::deque<Eigen::Vector3d> recent_path_directions_;  // Recent N path directions
  std::deque<double> recent_path_timestamps_;           // Corresponding timestamps
  static constexpr int kMaxHistorySize = 8;             // INCREASED: Max history size (was 5, now 8)
  static constexpr double kHistoryDecayTime = 45.0;     // INCREASED: History decay time in seconds (was 30.0, now 45.0)
