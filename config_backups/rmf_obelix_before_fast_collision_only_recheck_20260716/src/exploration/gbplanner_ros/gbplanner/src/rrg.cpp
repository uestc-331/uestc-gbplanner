...
bool Rrg::validatePathOnCurrentMap(
  ...
  collision_free = isPathCollisionFree(
      path, robot_box_size_, stop_at_unknown_voxel, true,
      "preplan/latest_map_recheck");
  clearance_safe =
      computePathClearanceCost(path, min_clearance, clearance_cost);
  if (!std::isfinite(min_clearance)) min_clearance = -1.0;

  if (!collision_free) {
    reason = "collision_or_unknown";
  } else if (!clearance_safe) {
    reason = "clearance_below_minimum";
  } else {
    reason = "valid";
  }
  ...
  ROS_WARN(
      ...
      "reason=%s",
      collision_free && clearance_safe, collision_free, clearance_safe,
      min_clearance, clearance_cost, path.size(), stop_at_unknown_voxel,
      reason.c_str());
  return collision_free && clearance_safe;
}
