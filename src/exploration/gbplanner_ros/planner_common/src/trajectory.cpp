...
double Trajectory::computeDistanceBetweenTrajectoryAndDirection(
  ...
  if (!std::isfinite(path_length) || path_length <= 1e-6) {
    // Very short paths can collapse to a single interpolated sample. Treat the
    // direction cost as neutral instead of returning infinity, which can turn a
    // zero-weight direction penalty into NaN.
    path_length = getPathLength(path);
    if (!std::isfinite(path_length) || path_length <= 1e-6) return 0.0;
    path_intp = path;
  }
  ...
  if (!direction.allFinite() || direction.norm() <= 1e-6) return 0.0;
  direction.normalize();

  std::vector<Eigen::Vector3d> path_ref;
  int n = std::max(1, static_cast<int>(std::ceil(path_length / discrete_length)));
  path_ref.reserve(n + 1);
  for (int i = 0; i <= n; ++i) {
    double step = std::min(path_length, i * discrete_length);
    Eigen::Vector3d ei = p0 + direction * step;
    path_ref.push_back(ei);
  }

  // DTW distance.
  double dtw_dist = computeDTWDistance(path_intp, path_ref);
  if (!std::isfinite(dtw_dist)) return 0.0;
  // Scale with length or length square
  if (scale_with_length) {
    double denom = path_length * path_length;
    if (denom > 1e-6) dtw_dist /= denom;
  }
