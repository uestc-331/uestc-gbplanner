...
bool PlanningParams::loadParams(std::string ns) {
  ...
  if (!ros::param::get(param_name, path_history_penalty)) {
    path_history_penalty = 1.0;  // default history penalty coefficient
  ...
  if (!ros::param::get(param_name, path_history_size)) {
    path_history_size = 5;  // default number of recent paths to remember
  ...
  if (!ros::param::get(param_name, path_history_decay_time)) {
    path_history_decay_time = 30.0;  // default decay time in seconds
  ...
  if (!ros::param::get(param_name, min_unknown_voxels_per_meter)) {
    min_unknown_voxels_per_meter = 50.0;  // default minimum unknown voxel density
