...
bool PlannerControlInterface::validateCachedPathOnLatestMap(
    const std::vector<geometry_msgs::Pose>& path, std::string& reason) {
  ...
  const double dx = current_pose_.position.x - path.front().position.x;
  const double dy = current_pose_.position.y - path.front().position.y;
  const double dz = current_pose_.position.z - path.front().position.z;
  const double gap = std::sqrt(dx * dx + dy * dy + dz * dz);
  if (gap > 0.05) {
    validate_srv.request.path.insert(validate_srv.request.path.begin(),
                                     current_pose_);
  } else {
    validate_srv.request.path.front() = current_pose_;
  }
  ...
  ROS_WARN(
      ...
      validate_srv.response.clearance_cost, gap, elapsed,
