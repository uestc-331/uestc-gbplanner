...
VoxelStatus MapManagerVoxblox<SDFServerType, SDFVoxelType>::getPathStatus(
  ...
  if(line_status_method_ == 1) {
    ...
    Eigen::Vector3d full_edge_vec = end - start;
    if (full_edge_vec.norm() < kEdgeEps) {
      return getBoxStatus(start, box_size, stop_at_unknown_voxel);
    }

    Eigen::Vector3d edge_vec = full_edge_vec;
    edge_vec(2) = 0.0;
    double edge_len = edge_vec.norm();
    if (edge_len < kEdgeEps) {
      Eigen::Vector3d step_dir = full_edge_vec.normalized();
      for (double dx = 0.0; dx <= full_edge_vec.norm(); dx += voxel_size) {
        Eigen::Vector3d vox_center = start + step_dir * dx;
    ...
    Eigen::Matrix3d unitx2Edge = Eigen::Matrix3d::Identity();
    ...
    if (std::abs(angle) > kEdgeEps) {
      Eigen::Vector3d rot_axis = edge_vec.cross(Eigen::Vector3d::UnitX());
      if (rot_axis.norm() < kEdgeEps) {
        unitx2Edge = Eigen::AngleAxisd(M_PI, Eigen::Vector3d::UnitZ());
      } else {
