...
Visualization::Visualization(const ros::NodeHandle& nh,
  ...
  planning_diagnostics_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>("vis/planning_diagnostics",
                                                     10, true);
  root_edge_diagnostics_pub_ =
      nh_.advertise<visualization_msgs::MarkerArray>(
          "vis/root_edge_diagnostics", 10, true);
  sensor_fov_pub_ =
...
void Visualization::visualizeRootEdgeDiagnostics(
    const Eigen::Vector3d& root,
    const std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>>& segments,
    const std::vector<VoxelStatus>& statuses, const std::string& summary) {
  if (segments.size() != statuses.size()) return;
  ...
  line.header.frame_id = world_frame_id;
  line.ns = "root_edge_segments";
  line.id = 0;
  ...
  for (size_t index = 0; index < segments.size(); ++index) {
    ...
    start.x = segments[index].first.x();
    start.y = segments[index].first.y();
    start.z = segments[index].first.z();
    geometry_msgs::Point end;
    end.x = segments[index].second.x();
    end.y = segments[index].second.y();
    end.z = segments[index].second.z();
    std_msgs::ColorRGBA color;
    color.a = 0.95;
    if (statuses[index] == VoxelStatus::kFree) {
      color.r = 0.0;
      color.g = 1.0;
      color.b = 0.1;
    } else if (statuses[index] == VoxelStatus::kOccupied) {
      color.r = 1.0;
      color.g = 0.05;
      color.b = 0.0;
    } else {
  ...
  visualization_msgs::Marker text_marker;
  text_marker.header = line.header;
  text_marker.ns = "root_edge_summary";
  text_marker.id = 1;
  text_marker.action = visualization_msgs::Marker::ADD;
  text_marker.type = visualization_msgs::Marker::TEXT_VIEW_FACING;
  text_marker.pose.position.x = root.x();
  text_marker.pose.position.y = root.y();
  text_marker.pose.position.z = root.z() + 1.35;
  text_marker.pose.orientation.w = 1.0;
  ...
  text_marker.color.a = 1.0;
  text_marker.text = summary;
  text_marker.lifetime = ros::Duration(0);
  marker_array.markers.push_back(text_marker);

  root_edge_diagnostics_pub_.publish(marker_array);
}
...
void Visualization::visualizeOptimizedPath(
    const std::vector<geometry_msgs::Pose>& path) {
  if (optimized_path_pub_.getNumSubscribers() < 1 || path.empty()) return;
  ...
  line.color.b = 1.0;
  line.color.a = 1.0;
  line.lifetime = ros::Duration(shortest_paths_lifetime);
...
{
  // Visualize as geometry_msgs::PoseArray
  if (viewpoints_pub_.getNumSubscribers() < 1) return;
