...
bool PlanningParams::loadParams(std::string ns) {
  ...
  if (!ros::param::get(param_name, path_optimizer_clearance_weight)) {
    path_optimizer_clearance_weight = 0.8;
  ...
  if (!ros::param::get(param_name, path_optimizer_anchor_weight)) {
    path_optimizer_anchor_weight = 0.4;
  ...
  if (!ros::param::get(param_name, path_optimizer_max_deviation)) {
    path_optimizer_max_deviation = 0.60;
  ...
  if (!ros::param::get(param_name, path_optimizer_gradient_step)) {
    path_optimizer_gradient_step = 0.15;
  ...
  param_name = ns + "/root_edge_relax_enable";
  if (!ros::param::get(param_name, root_edge_relax_enable)) {
    root_edge_relax_enable = false;
    ROSPARAM_WARN(param_name, root_edge_relax_enable);
  }
  param_name = ns + "/root_edge_relax_distance";
  if (!ros::param::get(param_name, root_edge_relax_distance)) {
    root_edge_relax_distance = 0.8;
    ROSPARAM_WARN(param_name, root_edge_relax_distance);
  }
  ...
  path_optimizer_max_deviation = std::max(0.0, path_optimizer_max_deviation);
  path_optimizer_gradient_step = std::max(0.01, path_optimizer_gradient_step);
  path_optimizer_min_clearance =
      std::max(wall_clearance_min, path_optimizer_min_clearance);
  path_optimizer_fix_start_points = std::max(1, path_optimizer_fix_start_points);
  root_edge_relax_distance = std::max(0.0, root_edge_relax_distance);
  ...
  param_val.clear();
  param_name = ns + "/global_bound_robot_size";
  if ((!ros::param::get(param_name, param_val)) || (param_val.size() != 3) ||
      (param_val[0] <= 0.0) || (param_val[1] <= 0.0) ||
      (param_val[2] <= 0.0)) {
    global_bound_robot_size.setZero();
    ROSPARAM_WARN(param_name, "unset; using the active collision box");
  } else {
    global_bound_robot_size << param_val[0], param_val[1], param_val[2];
  }

  param_name = ns + "/global_bound_tolerance";
  if (!ros::param::get(param_name, global_bound_tolerance)) {
    global_bound_tolerance = 0.0;
    ROSPARAM_WARN(param_name, global_bound_tolerance);
  }
  if (global_bound_tolerance < 0.0) {
    ROS_WARN("[PlanningParams] global_bound_tolerance %.3f is negative; "
             "using 0.0",
             global_bound_tolerance);
    global_bound_tolerance = 0.0;
  }

  param_name = ns + "/preplan_recheck_near_robot_relax_enable";
  if (!ros::param::get(param_name,
                       preplan_recheck_near_robot_relax_enable)) {
    preplan_recheck_near_robot_relax_enable = false;
    ROSPARAM_WARN(param_name, preplan_recheck_near_robot_relax_enable);
  }

  param_name = ns + "/preplan_recheck_near_robot_relax_distance";
  if (!ros::param::get(param_name,
                       preplan_recheck_near_robot_relax_distance)) {
    preplan_recheck_near_robot_relax_distance = 0.5;
    ROSPARAM_WARN(param_name, preplan_recheck_near_robot_relax_distance);
  }
  preplan_recheck_near_robot_relax_distance =
      std::max(0.0, preplan_recheck_near_robot_relax_distance);
