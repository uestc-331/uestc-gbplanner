...
bool Rrg::sampleVertex(Vertex& vertex) {
  ...
  while (!found && while_thres--) {
    ...
    MapManager::VoxelStatus box_status = map_manager_->getBoxStatus(
        Eigen::Vector3d(state[0], state[1], state[2]) +
            robot_params_.center_offset,
        robot_box_size_, true);
    if (MapManager::VoxelStatus::kFree == box_status) {
      random_sampler_.pushSample(state, true);  // for debug purpose.
      found = true;
    } else {
      stat_->num_vertices_fail++;
      random_sampler_.pushSample(state, false);
      ROS_DEBUG_COND(global_verbosity >= Verbosity::DEBUG && attempts > 500, 
                     "[RRG] Sample failed: box_status=%d at (%.2f,%.2f,%.2f) after %d attempts", 
                     (int)box_status, state[0], state[1], state[2], attempts);
    }
  }
  if (!found && attempts >= 1000) {
    ROS_WARN_COND(global_verbosity >= Verbosity::WARN, 
                  "[RRG] Failed to sample vertex after %d attempts! Map may be too dense or sampling space too small", 
                  attempts);
  }
  vertex.state = state;
...
void Rrg::expandGraph(std::shared_ptr<GraphManager> graph_manager,
  ...
  if (robot_params_.type == RobotType::kAerialRobot) {
    ...
    ROS_DEBUG_COND(global_verbosity >= Verbosity::DEBUG, 
                   "[RRG] Path status check: start=(%.2f,%.2f,%.2f) end=(%.2f,%.2f,%.2f) status=%d", 
                   start_pos[0], start_pos[1], start_pos[2], 
                   end_pos[0], end_pos[1], end_pos[2], (int)vs);
    if (MapManager::VoxelStatus::kFree == vs) {
      admissible_edge = true;
    } else {
      ROS_DEBUG_COND(global_verbosity >= Verbosity::DEBUG, 
                     "[RRG] Path not free: status=%d (0=Free, 1=Occupied, 2=Unknown)", (int)vs);
    }
  ...
  if (admissible_edge) {
    ...
    ++rep.num_edges_added;
    ROS_DEBUG_COND(global_verbosity >= Verbosity::DEBUG, 
                   "[RRG] ✓ Added vertex %d at (%.2f, %.2f, %.2f), edge length=%.2f, from vertex %d", 
                   new_vertex->id, new_state[0], new_state[1], new_state[2], direction_norm, nearest_vertex->id);
...
Rrg::GraphStatus Rrg::buildGraph() {
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] ========== buildGraph() Started ==========");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[RRG] Planning params: max_vertices=%d, max_edges=%d, max_loops=%d",
                (int)planning_num_vertices_max_, (int)planning_num_edges_max_, (int)planning_params_.num_loops_max);
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Root vertex: (%.2f, %.2f, %.2f, %.2f)", 
                root_vertex_->state[0], root_vertex_->state[1], 
                root_vertex_->state[2], root_vertex_->state[3]);
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Local space: min=(%.2f,%.2f,%.2f) max=(%.2f,%.2f,%.2f)", 
                local_space_params_.min_val[0], local_space_params_.min_val[1], local_space_params_.min_val[2],
                local_space_params_.max_val[0], local_space_params_.max_val[1], local_space_params_.max_val[2]);
  
  // Check map manager status
  bool map_ready = map_manager_->getStatus();
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Map manager status: %s", map_ready ? "READY" : "NOT_READY");
  ...
  int fail_count_kdTree = 0;
  int fail_count_shortEdge = 0;
  int fail_count_collision = 0;
  int fail_count_geofence = 0;
  int fail_count_sample = 0;
  int success_count = 0;

  ROS_INFO_COND(global_verbosity >= Verbosity::INFO,
                "[RRG] Starting graph building loop (max_loops=%d, max_vertices=%d, max_edges=%d)",
                (int)planning_params_.num_loops_max, (int)planning_num_vertices_max_, (int)planning_num_edges_max_);
  ...
  while ((loop_count++ < planning_params_.num_loops_max) &&
    ...
    if (planning_params_.type == PlanningModeType::kAdaptiveExploration) {
      if (!sampleVertex(random_sampler_adaptive_, root_vertex_->state,
        ...
        if (loop_count % 100 == 0) {
          ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                        "[RRG] Loop %d: Sample failed (total sample fails: %d)", 
                        loop_count, fail_count_sample);
        }
        continue;
      }
    } else {
    ...
    if (rep.status == ExpandGraphStatus::kSuccess) {
      ...
      success_count++;
      if (success_count % 10 == 0 || loop_count % 100 == 0) {
        ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                      "[RRG] Loop %d: ✓ Added %d vertices, %d edges. Total: %d vertices, %d edges (success: %d)", 
                      loop_count, rep.num_vertices_added, rep.num_edges_added, num_vertices, num_edges, success_count);
      }
    } else {
      ...
      switch (rep.status) {
        ...
        case ExpandGraphStatus::kErrorShortEdge:
          fail_count_shortEdge++;
          break;
        case ExpandGraphStatus::kErrorCollisionEdge:
          fail_count_collision++;
          break;
        case ExpandGraphStatus::kErrorGeofenceViolated:
          fail_count_geofence++;
          break;
        default:
          break;
      ...
      if (loop_count % 100 == 0) {
        ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                      "[RRG] Loop %d: ✗ Failed (status=%d). Stats - Success: %d, KdTree: %d, ShortEdge: %d, Collision: %d, Geofence: %d, Sample: %d", 
                      loop_count, (int)rep.status, success_count, fail_count_kdTree, 
                      fail_count_shortEdge, fail_count_collision, fail_count_geofence, fail_count_sample);
      }
    }
    ...
    if ((loop_count >= planning_params_.num_loops_cutoff) &&
      ...
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN, 
                    "[RRG] Early termination: loop_count=%d >= cutoff=%.0f but only %d vertices", 
                    loop_count, (double)planning_params_.num_loops_cutoff, local_graph_->getNumVertices());
      break;
    }
  }
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] ========== buildGraph() Completed ==========");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Final graph: %d vertices, %d edges, %d loops", 
                num_vertices, num_edges, loop_count);
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Success rate: %d successes out of %d attempts (%.1f%%)", 
                success_count, loop_count, loop_count > 0 ? 100.0 * success_count / loop_count : 0.0);
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Failure breakdown: KdTree=%d, ShortEdge=%d, Collision=%d, Geofence=%d, Sample=%d", 
                fail_count_kdTree, fail_count_shortEdge, fail_count_collision, fail_count_geofence, fail_count_sample);
...
Rrg::GraphStatus Rrg::evaluateGraph() {
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] ========== evaluateGraph() Started ==========");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Graph has %d vertices, %d edges", 
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Found %zu leaf vertices", leaf_vertices.size());
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Computing exploration gain for vertices...");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Gain params: leafs_only=%d, clustering=%d, unknown_voxel_gain=%.1f", 
                planning_params_.leafs_only_for_volumetric_gain, 
  ...
  // Check gain statistics
  double total_gain = 0;
  double max_gain = 0;
  int vertices_with_gain = 0;
  int frontier_count = 0;
  for (const auto& v : local_graph_->vertices_map_) {
    if (v.second->vol_gain.gain > 0) {
      vertices_with_gain++;
      total_gain += v.second->vol_gain.gain;
      if (v.second->vol_gain.gain > max_gain) {
        max_gain = v.second->vol_gain.gain;
      }
    }
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Exploration gain computation completed");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Gain stats: %d/%d vertices have gain, max_gain=%.2f, total_gain=%.2f, frontiers=%d", 
                vertices_with_gain, local_graph_->getNumVertices(), max_gain, total_gain, frontier_count);
  ...
  // Store detailed gain information for best path
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Evaluating %d leaf vertices for best path...", num_leaf_vertices);
    ...
    if (path_size > 1) {
      ...
      for (int ind = 0; ind < path_size; ++ind) {
        ...
        if (!inadmissible_edge) {
          ...
          // Only count unknown voxels from the leaf vertex (path[0]) to avoid double counting
          // Path vertices are close together and see overlapping unknown voxels
          if (ind == 0) {
      ...
      // [Opt1] 从已有的 path(vector<Vertex*>) 直接提取坐标，避免重复调用 getShortestPath
      std::vector<Eigen::Vector3d> path_list;
      path_list.reserve(path_size);
      for (auto* v : path) {
        path_list.emplace_back(v->state.x(), v->state.y(), v->state.z());
      }
      // [Opt2] 插值步长从 0.2m 增大到 1.0m，DTW 计算量从 O(L²) 降至 O((L/5)²)
      double fw_ratio =
          Trajectory::computeDistanceBetweenTrajectoryAndDirection(
              path_list, exploring_direction_, 1.0, true);
      double path_gain_before_direction_penalty = path_gain;
      path_gain *= exp(-lambda2 * fw_ratio);

      // ========== History Path Penalty (Anti-Oscillation Mechanism) ==========
      // Apply penalty for recently attempted path directions to prevent oscillation
      double history_penalty = 0.0;
      if (!recent_path_directions_.empty() && path_list.size() >= 2) {
        // Compute current path direction (from root to leaf)
        Eigen::Vector3d current_path_direction =
            (path_list[0] - path_list.back()).normalized();

        double current_time = ros::Time::now().toSec();
        double total_weight = 0.0;

        // Compare with each historical path direction
        for (size_t h = 0; h < recent_path_directions_.size(); ++h) {
          double time_diff = current_time - recent_path_timestamps_[h];
          double time_weight = exp(-time_diff / kHistoryDecayTime);  // Exponential decay

          // Compute direction similarity (dot product)
          // FIXED: Penalize paths that DIFFER from historical direction
          // similarity = 1 means same direction (no penalty)
          // similarity = 0 means perpendicular (medium penalty)
          // similarity = -1 means opposite direction (maximum penalty)
          double direction_similarity =
              current_path_direction.dot(recent_path_directions_[h]);

          // Convert similarity to dissimilarity: dissimilar paths get penalized
          // dissimilarity ranges from 0 (same direction) to 2 (opposite direction)
          double dissimilarity = 1.0 - direction_similarity;  // 0 to 2
          dissimilarity = std::max(0.0, dissimilarity);  // Ensure non-negative

          history_penalty += dissimilarity * time_weight;
          total_weight += time_weight;
        }

        // Normalize by total weight
        if (total_weight > 0.0) {
          history_penalty /= total_weight;
        }

        // Apply history penalty to path gain
        double lambda_history = planning_params_.path_history_penalty;
        double path_gain_before_history = path_gain;
        path_gain *= exp(-lambda_history * history_penalty);

        ROS_DEBUG("[RRG] Path %d: history_penalty=%.3f, gain: %.2f -> %.2f",
                  id, history_penalty, path_gain_before_history, path_gain);
      }
      // ========== End of History Path Penalty ==========
      ...
      if (path_gain > best_gain) {
        ...
        for (int ind = 1; ind < path_size; ++ind) {
          Eigen::Vector3d diff = path[ind]->state.head(3) - path[ind-1]->state.head(3);
          total_path_length += diff.norm();
        }
        ...
        ROS_WARN("[RRG] New best path: id=%d, final_gain=%.4f, vol_gain_before_penalty=%.4f, path_length=%.2f, unknown_voxels=%d, fw_ratio=%.3f", 
                       id, path_gain, path_gain_before_direction_penalty, total_path_length, num_unknown_voxels, fw_ratio);
      }
      if (path_gain > 0) {
        paths_with_positive_gain++;
      }
    }
  }
  
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Path evaluation: %d/%d paths evaluated, %d with positive gain", 
                paths_evaluated, num_leaf_vertices, paths_with_positive_gain);
  
  // Output detailed gain information (always show, not conditional)
  if (best_gain > 0) {
    ROS_WARN("[RRG] ========== Best Path Gain Details ==========");
    ROS_WARN("[RRG] Final gain: %.4f", best_gain);
    ROS_WARN("[RRG] Volumetric gain (before penalties): %.4f", best_vol_gain_before_penalty);
    ROS_WARN("[RRG] Path length: %.2f m (penalty factor: %.4f)", 
                  best_path_length, exp(-planning_params_.path_length_penalty * best_path_length));
    ROS_WARN("[RRG] Direction penalty (fw_ratio): %.4f (penalty factor: %.4f)", 
                  best_fw_ratio, exp(-planning_params_.path_direction_penalty * best_fw_ratio));
    ROS_WARN("[RRG] Unknown voxels: %d (gain per voxel: %.2f)", 
                  best_num_unknown_voxels, planning_params_.unknown_voxel_gain);
    ROS_WARN("[RRG] Path length penalty: %.3f, Direction penalty: %.3f", 
                  planning_params_.path_length_penalty, planning_params_.path_direction_penalty);
    ROS_WARN("[RRG] ============================================");
  }
  ...
  // Check if exploration should continue with stricter conditions
  // Using thresholds from configuration file
  double min_gain_threshold = planning_params_.min_gain_threshold;
  double min_path_length = planning_params_.min_path_length;
  double min_unknown_voxels_per_meter = planning_params_.min_unknown_voxels_per_meter;
  
  // Calculate unknown voxel density
  double unknown_voxels_per_meter = (best_path_length > 0.1) ? 
      (best_num_unknown_voxels / best_path_length) : 0.0;
  
  // Stricter condition: must meet all criteria
  bool exploration_worthwhile = (best_gain > min_gain_threshold) && 
                                (best_path_length > min_path_length) &&
                                (unknown_voxels_per_meter > min_unknown_voxels_per_meter);
  
  if (best_gain > 0 && exploration_worthwhile) {
    // create a branch
    std::vector<int> path;
    ...
    // Store the chosen path direction to prevent oscillation in future decisions
    std::vector<Eigen::Vector3d> chosen_path_list;
    local_graph_->getShortestPath(best_path_id, local_graph_rep_, true, chosen_path_list);

    if (chosen_path_list.size() >= 2) {
      Eigen::Vector3d chosen_direction =
          (chosen_path_list[0] - chosen_path_list.back()).normalized();

      recent_path_directions_.push_back(chosen_direction);
      recent_path_timestamps_.push_back(ros::Time::now().toSec());

      // Limit history size
      if (recent_path_directions_.size() > kMaxHistorySize) {
        recent_path_directions_.pop_front();
        recent_path_timestamps_.pop_front();
      }

      ROS_INFO("[RRG] Path direction added to history: [%.2f, %.2f, %.2f], history_size=%zu",
               chosen_direction.x(), chosen_direction.y(), chosen_direction.z(),
               recent_path_directions_.size());
    }
    // ========== End of Recording Path Direction ==========

    ROS_WARN("[RRG] Path ACCEPTED: best_vertex_->id=%d, path_length=%.2f m, gain=%.2f",
             best_vertex_->id, best_path_length, best_gain);
    //
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                  "[RRG] Best path found: gain=%.4f, path_id=%d, path_length=%zu waypoints (%.2f m)", 
                  best_gain, best_path_id, path.size(), best_path_length);
    ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                  "[RRG] Evaluated %d leaf vertices, best gain=%.4f, unknown_voxels=%d", 
                  num_leaf_vertices, best_gain, best_num_unknown_voxels);
    ...
    if (best_gain > 0) {
      ROS_WARN("[RRG] Exploration path rejected: gain=%.4f (min=%.1f), path_length=%.2f m (min=%.1f), "
               "unknown_voxels=%d, density=%.1f voxels/m (min=%.1f)",
               best_gain, min_gain_threshold, best_path_length, min_path_length,
               best_num_unknown_voxels, unknown_voxels_per_meter, min_unknown_voxels_per_meter);
    } else {
      ROS_WARN_COND(global_verbosity >= Verbosity::WARN, 
                    "[RRG] No positive gain found! Evaluated %d leaf vertices, best_gain=%.4f", 
                    num_leaf_vertices, best_gain);
    }
  ...
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] ========== evaluateGraph() Completed ==========");
  ROS_INFO_COND(global_verbosity >= Verbosity::INFO, 
                "[RRG] Final status: %d (0=OK, 1=NO_GAIN, 2=NOT_OK)", (int)gstatus);
...
std::vector<geometry_msgs::Pose> Rrg::getBestPath(std::string tgt_frame,
  ...
  if (planning_params_.auto_homing_enable) {
    ...
    if (time_budget_remaining <= 0.0) {
      ROS_WARN("[RRG] RAN OUT OF TIME BUDGET: elapsed=%.1f s, limit=%.1f s --> STOP HERE.", 
               time_elapsed, planning_params_.time_budget_limit);
      return homing_path;
    }
  ...
  if (best_vertex_ == nullptr) {
    ROS_WARN("[RRG] getBestPath: best_vertex_ is nullptr!");
    return ret;
  }
  int id = best_vertex_->id;
  if (id == 0) {
    ROS_WARN("[RRG] getBestPath: best_vertex_->id is 0, returning empty path");
    return ret;
  }
  ...
  if (total_len <= len_min_thres) {
    ROS_WARN("[RRG] getBestPath: Best path is too short: %.3f m (min=%.3f m), rejecting path", 
             total_len, len_min_thres);
    return ret;
  }
...
void Rrg::timerCallback(const ros::TimerEvent& event) {
  ...
  constexpr double kAlpha = 0.85;  // INCREASED: favor historical direction to maintain consistency (0.85 = 15% update rate)
