...
bool Rrg::isFinalPathSafeWithDirtyRootEscape(
  ...
  if (root_status == VoxelStatus::kFree) {
    ROS_WARN(
        "[RRG][DIRTY_ROOT_ESCAPE] %s rejected: strict final check failed but "
        "root is free.",
        log_context.c_str());
    return false;
  }
  ...
  if (!isPathCollisionFree(trimmed_path, robot_size, true, true,
    ...
    ROS_WARN(
        ...
        "trim_dist=%.2f trim=[%.2f %.2f %.2f]",
        log_context.c_str(), trim_dist, trim_point.x(), trim_point.y(),
        trim_point.z());
    return false;
  }
  ...
  ROS_WARN(
      "[RRG][DIRTY_ROOT_ESCAPE] %s accepted dirty root escape: root_status=%s "
