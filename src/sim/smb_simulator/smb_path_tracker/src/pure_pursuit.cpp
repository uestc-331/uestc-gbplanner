	...
	as_(nh_, "pci_output_path", boost::bind(&PurePursuitController::actionGoalCallback, this, _1), false)
{
	...
	path_sub_ = nh_.subscribe("path", 100, &PurePursuitController::pathCallback, this);
	loadParams();
	path_to_exec_.clear();
...
void PurePursuitController::pathCallback(const nav_msgs::Path &path)
{
	path_to_exec_.clear();
	double z_offset = path.poses[0].pose.position.z - current_pose_.position.z;
	for(int i=0;i<path.poses.size();++i)
	{
		geometry_msgs::PoseStamped pose = path.poses[i];
		pose.pose.position.z -= z_offset;
		path_to_exec_.push_back(pose);
	}
...
void PurePursuitController::loadParams()
{
  ...
  param_name = ns + "/use_action";
  if (!ros::param::get(param_name, use_action_)) {
    use_action_ = true;
  }
