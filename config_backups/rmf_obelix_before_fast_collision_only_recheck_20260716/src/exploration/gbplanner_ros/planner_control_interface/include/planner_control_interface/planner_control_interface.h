...
class PlannerControlInterface {
 ...
 private:
  ...
  ros::Publisher preplan_status_pub_;
  ...
  enum class PreplanState { kDisabled, kIdle, kPlanning, kCached, kFailed };

  bool preplan_enable_ = false;
  double preplan_trigger_remaining_time_ = 4.0;
  double preplan_min_current_remaining_distance_ = 1.0;
  double preplan_retry_interval_ = 1.5;
  int preplan_min_path_size_ = 2;
  double preplan_cache_max_age_ = 15.0;
  double preplan_start_tolerance_ = 0.60;
  bool preplan_adaptive_lead_time_enable_ = true;
  double preplan_lead_time_min_ = 3.0;
  double preplan_lead_time_max_ = 12.0;
  double preplan_lead_time_percentile_ = 0.90;
  double preplan_lead_time_margin_ = 1.0;
  int preplan_latency_history_size_ = 20;
  bool preplan_latest_map_recheck_enable_ = true;
  bool preplan_recheck_stop_at_unknown_ = true;

  mutable std::mutex preplan_mutex_;
  std::thread preplan_thread_;
  bool preplan_worker_running_ = false;
  uint64_t preplan_epoch_ = 0;
  uint64_t active_execution_sequence_ = 0;
  uint64_t preplan_execution_sequence_ = 0;
  PreplanState preplan_state_ = PreplanState::kIdle;
  ros::Time preplan_last_attempt_time_;
  ros::Time preplan_cached_time_;
  geometry_msgs::Pose preplan_root_pose_;
  planner_msgs::planner_srv::Response preplan_response_;
  std::string preplan_last_reason_ = "idle";
  ros::Time preplan_last_marker_time_;
  std::vector<double> preplan_latency_history_;

  ros::ServiceClient preplan_planner_client_;
  ros::ServiceClient preplan_validate_path_client_;

  static const char* preplanStateName(PreplanState state);
  void reapPreplanThread();
  bool isPreplanWorkerRunning() const;
  bool hasCachedPreplan() const;
  void invalidatePreplan(const std::string& reason);
  void maybeStartPreplan();
  void preplanWorker(uint64_t epoch, uint64_t execution_sequence,
                     const geometry_msgs::Pose& root_pose);
  double getAdaptivePreplanLeadTime() const;
  void recordPreplanLatency(double latency);
  bool validateCachedPathOnLatestMap(
      const std::vector<geometry_msgs::Pose>& path, std::string& reason);
  bool tryExecuteCachedPreplan();
  void publishPreplanStatus(const std::string& reason);
