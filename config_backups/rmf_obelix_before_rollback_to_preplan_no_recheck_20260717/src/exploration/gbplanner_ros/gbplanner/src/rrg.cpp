...
bool Rrg::sampleVertex(Vertex& vertex) {
  ...
  BoundedSpaceParams reduced_global_space;
  if (!getReducedGlobalPlanningSpace(robot_box_size_, reduced_global_space)) {
    ROS_ERROR_THROTTLE(2.0, "[RRG][GLOBAL_BOUND] Invalid reduced Global "
                            "planning space; cannot sample vertices.");
    return false;
  }
...
void Rrg::expandGraph(std::shared_ptr<GraphManager> graph_manager,
  ...
  if (robot_params_.type == RobotType::kAerialRobot) {
    const bool root_edge = nearest_vertex->id == 0;
    if ((root_edge && isRootEdgeSafe(start_pos, end_pos, "expand_nearest")) ||
        (!root_edge && isPathStatusFreeWithEndpointTolerance(
                           start_pos, end_pos, robot_box_size_, true, false,
...
void Rrg::expandGraph(std::shared_ptr<GraphManager> graph_manager,
  ...
  if (robot_params_.type == RobotType::kAerialRobot) {
    const bool root_edge = nearest_vertex->id == 0;
    if ((root_edge &&
         isRootEdgeSafe(start_pos, end_pos, "expand_vertex_nearest")) ||
        (!root_edge && isPathStatusFreeWithEndpointTolerance(
                           start_pos, end_pos, robot_box_size_, true, false,
...
Rrg::GraphStatus Rrg::buildGraph() {
  ...
  ROS_INFO("[RRG][BUILD] root=[%.2f %.2f %.2f yaw=%.2f] current=[%.2f %.2f %.2f yaw=%.2f]",
                ...
                current_state_[3]);
  const Eigen::Vector3d global_bound_size =
      getGlobalBoundCheckSize(robot_box_size_);
  const bool root_inside_global_bound = isPointInsideGlobalPlanningBounds(
      root_vertex_->state.head(3), robot_box_size_, false,
      "buildGraph/root");
  ROS_INFO("[RRG][GLOBAL_BOUND_ROOT] inside=%d fixed_size=(%.3f, %.3f, %.3f) "
           "tolerance=%.3f root=(%.3f, %.3f, %.3f)",
           root_inside_global_bound, global_bound_size.x(),
           global_bound_size.y(), global_bound_size.z(),
           planning_params_.global_bound_tolerance, root_vertex_->state[0],
           root_vertex_->state[1], root_vertex_->state[2]);
...
std::vector<geometry_msgs::Pose> Rrg::getBestPathSimplified()
{
  ...
  if (planning_params_.path_optimizer_enable) {
    const std::vector<geometry_msgs::Pose> original_path = ret;
    std::vector<geometry_msgs::Pose> optimized_path;
    if (optimizeLocalPath(original_path, optimized_path)) {
      ret = optimized_path;
      visualization_->visualizeOptimizedPath(ret);
    } else {
      ret = original_path;
      ROS_WARN(
          "[RRG][OPT] simplified local path optimization rejected; "
          "executing original path size=%zu",
          ret.size());
    }
  }
...
std::vector<geometry_msgs::Pose> Rrg::getBestPath(std::string tgt_frame,
  ...
  if (planning_params_.path_optimizer_enable &&
    ...
    if (optimizeLocalPath(original_path, optimized_path)) {
      ...
      ret = original_path;
      ROS_WARN(
          "[RRG][OPT] optimization rejected; executing original local path "
          "size=%zu",
          ret.size());
    }
  }
...
Eigen::Vector3d Rrg::getGlobalBoundCheckSize(
    const Eigen::Vector3d& fallback_size) const {
  const Eigen::Vector3d& configured_size =
      planning_params_.global_bound_robot_size;
  if ((configured_size.array() > 0.0).all()) return configured_size;
  return fallback_size;
}

bool Rrg::getReducedGlobalPlanningSpace(
    const Eigen::Vector3d& fallback_size,
    BoundedSpaceParams& reduced_space) const {
  reduced_space = global_space_params_;
  const Eigen::Vector3d bound_size =
      getGlobalBoundCheckSize(fallback_size);
  const double tolerance =
      std::max(0.0, planning_params_.global_bound_tolerance);

  bool valid_bound = true;
  if (reduced_space.type == BoundedSpaceType::kCuboid) {
    const Eigen::Vector3d margin =
        (0.5 * bound_size.array() - tolerance).max(0.0).matrix();
    reduced_space.min_val += margin;
    reduced_space.max_val -= margin;
    valid_bound =
        (reduced_space.min_val.array() <= reduced_space.max_val.array()).all();
  } else if (reduced_space.type == BoundedSpaceType::kSphere) {
    const double margin = std::max(0.0, 0.5 * bound_size.norm() - tolerance);
    reduced_space.radius -= margin;
    valid_bound = reduced_space.radius >= 0.0;
  }

  Eigen::Vector3d bound_center = reduced_space.getCenter();
  reduced_space.setCenter(bound_center, false);
  return valid_bound;
}
...
bool Rrg::isPointInsideGlobalPlanningBounds(const Eigen::Vector3d& point,
  ...
  BoundedSpaceParams reduced_global_space;
  const Eigen::Vector3d bound_size = getGlobalBoundCheckSize(robot_size);
  const bool valid_bound =
      getReducedGlobalPlanningSpace(robot_size, reduced_global_space);
  ...
  if (!valid_bound) {
    if (log_failure) {
      ROS_WARN(
          ...
          "radius=%.3f",
          log_context.c_str(), bound_size.x(), bound_size.y(), bound_size.z(),
  ...
  if (!inside && log_failure) {
    ROS_WARN(
        ...
        bound_size.x(), bound_size.y(), bound_size.z(),
        reduced_global_space.min_val.x(), reduced_global_space.min_val.y(),
        reduced_global_space.min_val.z(), reduced_global_space.max_val.x(),
        reduced_global_space.max_val.y(), reduced_global_space.max_val.z());
  }
...
bool Rrg::validatePathOnCurrentMap(
    const std::vector<geometry_msgs::Pose>& path,
    bool stop_at_unknown_voxel, bool& collision_free, bool& clearance_safe,
    double& min_clearance, double& clearance_cost, std::string& reason) {
  ...
  const Eigen::Vector3d recheck_box_size =
      robot_params_.size + robot_params_.size_extension_min;
  const Eigen::Vector3d near_robot_box_size = robot_params_.size;
  const double near_robot_relax_distance =
      planning_params_.preplan_recheck_near_robot_relax_enable
          ? planning_params_.preplan_recheck_near_robot_relax_distance
          : 0.0;

  collision_free = isPathInsideGlobalPlanningBounds(
      path, recheck_box_size, true, "preplan/latest_map_recheck");
  double traversed_distance = 0.0;
  for (size_t i = 0; collision_free && i + 1 < path.size(); ++i) {
    const Eigen::Vector3d start(path[i].position.x, path[i].position.y,
                                path[i].position.z);
    const Eigen::Vector3d end(path[i + 1].position.x,
                              path[i + 1].position.y,
                              path[i + 1].position.z);
    const double segment_length = (end - start).norm();
    const bool near_robot =
        traversed_distance < near_robot_relax_distance;
    const Eigen::Vector3d& check_size =
        near_robot ? near_robot_box_size : recheck_box_size;
    const VoxelStatus status = map_manager_->getPathStatus(
        start, end, check_size, stop_at_unknown_voxel);
    if (VoxelStatus::kFree != status) {
      ROS_WARN(
          "[RRG][PATH_RECHECK_COLLISION] segment=%zu/%zu status=%s "
          "start=(%.3f, %.3f, %.3f) end=(%.3f, %.3f, %.3f) "
          "distance=%.3f near_robot=%d box=(%.3f, %.3f, %.3f)",
          i, path.size() - 1, voxelStatusToString(status), start.x(),
          start.y(), start.z(), end.x(), end.y(), end.z(),
          traversed_distance, near_robot, check_size.x(), check_size.y(),
          check_size.z());
      collision_free = false;
      break;
    }
  ...
  if (!collision_free) {
    reason = "collision_or_unknown";
  } else {
    reason = "valid";
  }

  ROS_WARN(
      "[RRG][PATH_RECHECK] valid=%d collision_free=%d clearance_safe=%d "
      "min_clearance=%.3f clearance_cost=%.3f size=%zu stop_unknown=%d "
      "box=(%.3f, %.3f, %.3f) near_box=(%.3f, %.3f, %.3f) "
      "near_dist=%.3f clearance_skipped=1 reason=%s",
      collision_free, collision_free, clearance_safe, min_clearance,
      clearance_cost, path.size(), stop_at_unknown_voxel,
      recheck_box_size.x(), recheck_box_size.y(), recheck_box_size.z(),
      near_robot_box_size.x(), near_robot_box_size.y(),
      near_robot_box_size.z(), near_robot_relax_distance, reason.c_str());
...
bool Rrg::isRootEdgeSafe(const Eigen::Vector3d& start,
                         const Eigen::Vector3d& end,
                         const std::string& log_context) {
  if (!planning_params_.root_edge_relax_enable) {
    return isPathStatusFreeWithEndpointTolerance(
        start, end, robot_box_size_, true, true, false, log_context);
  }

  const Eigen::Vector3d delta = end - start;
  const double length = delta.norm();
  if (length <= 1e-6) return false;
  const double near_length =
      std::min(length, planning_params_.root_edge_relax_distance);
  const Eigen::Vector3d split = start + delta * (near_length / length);
  const Eigen::Vector3d minimum_box =
      robot_params_.size + robot_params_.size_extension_min;

  const VoxelStatus near_status =
      map_manager_->getPathStatus(start, split, minimum_box, true);
  if (near_status != VoxelStatus::kFree) {
    ROS_INFO_THROTTLE(
        1.0,
        "[RRG][ROOT_EDGE] %s near segment rejected status=%s length=%.2f "
        "box=[%.2f %.2f %.2f]",
        log_context.c_str(), voxelStatusToString(near_status), near_length,
        minimum_box.x(), minimum_box.y(), minimum_box.z());
    return false;
  }

  if (near_length + 1e-6 < length) {
    const VoxelStatus far_status =
        map_manager_->getPathStatus(split, end, robot_box_size_, true);
    if (far_status != VoxelStatus::kFree) {
      ROS_INFO_THROTTLE(
          1.0,
          "[RRG][ROOT_EDGE] %s far segment rejected status=%s length=%.2f",
          log_context.c_str(), voxelStatusToString(far_status),
          length - near_length);
      return false;
    }
  }
  ...
  ROS_INFO_THROTTLE(
      1.0,
      "[RRG][ROOT_EDGE] %s accepted split check near=%.2f total=%.2f "
      "min_box=[%.2f %.2f %.2f]",
      log_context.c_str(), near_length, length, minimum_box.x(),
      minimum_box.y(), minimum_box.z());
  return true;
}

void Rrg::diagnoseBlockedRootEdges() {
  if (root_vertex_ == nullptr) return;

  constexpr int kDirectionCount = 16;
  constexpr int kDistanceCount = 4;
  constexpr double kDistanceStep = 0.20;
  const Eigen::Vector3d root =
      root_vertex_->state.head(3) + robot_params_.center_offset;
  const Eigen::Vector3d box_size =
      robot_params_.size + robot_params_.size_extension_min;
  std::vector<std::pair<Eigen::Vector3d, Eigen::Vector3d>> segments;
  std::vector<VoxelStatus> statuses;
  int free_count = 0;
  int occupied_count = 0;
  int unknown_count = 0;
  int fully_free_directions = 0;

  segments.reserve(kDirectionCount * kDistanceCount);
  statuses.reserve(kDirectionCount * kDistanceCount);
  for (int direction_index = 0; direction_index < kDirectionCount;
       ++direction_index) {
    const double angle =
        2.0 * M_PI * static_cast<double>(direction_index) /
        static_cast<double>(kDirectionCount);
    const Eigen::Vector3d direction(std::cos(angle), std::sin(angle), 0.0);
    bool direction_free = true;
    for (int distance_index = 0; distance_index < kDistanceCount;
         ++distance_index) {
      const double start_distance = distance_index * kDistanceStep;
      const double end_distance = (distance_index + 1) * kDistanceStep;
      const Eigen::Vector3d start = root + start_distance * direction;
      const Eigen::Vector3d end = root + end_distance * direction;
      const VoxelStatus status =
          map_manager_->getPathStatus(start, end, box_size, true);
    ...
    if (direction_free) ++fully_free_directions;
  }

  std::ostringstream summary;
  summary << "ROOT EDGE BLOCKED\n"
          << "green/free: " << free_count << "  red/occupied: "
          << occupied_count << "\n"
          << "yellow/unknown: " << unknown_count
          << "  free_dirs: " << fully_free_directions << "/"
          << kDirectionCount;
  visualization_->visualizeRootEdgeDiagnostics(root, segments, statuses,
                                               summary.str());
  ROS_WARN(
      "[RRG][ROOT_DIAG] root=[%.2f %.2f %.2f] box=[%.2f %.2f %.2f] "
      "directions=%d distances=[0.20 0.40 0.60 0.80] free=%d "
      "occupied=%d unknown=%d fully_free_dirs=%d",
      root.x(), root.y(), root.z(), box_size.x(), box_size.y(), box_size.z(),
      kDirectionCount, free_count, occupied_count, unknown_count,
      fully_free_directions);
}
...
bool Rrg::optimizeLocalPath(
  ...
  for (int iteration = 0;
    ...
    for (size_t index = first_movable; index + 1 < points.size(); ++index) {
      ...
      const Eigen::Vector3d clearance_force =
          estimateClearanceGradient(points[index]);
      ...
      Eigen::Vector3d direction =
          ...
          planning_params_.path_optimizer_clearance_weight * clearance_force +
          planning_params_.path_optimizer_anchor_weight * anchor_force;
      if (!direction.allFinite() || direction.norm() <= 1e-6) continue;

      direction.normalize();
      double step = planning_params_.path_optimizer_step_size;
      for (int attempt = 0; attempt < 4; ++attempt) {
        Eigen::Vector3d candidate = points[index] + step * direction;
        Eigen::Vector3d deviation = candidate - original_points[index];
  ...
  if (accepted_moves == 0) {
    ROS_WARN("[RRG][OPT] no safe point movement accepted time=%.4fs",
  ...
  for (size_t index = 0; index + 1 < path_optimized.size(); ++index) {
    ...
    if (direction.head<2>().norm() > 1e-3) {
      path_optimized[index].orientation =
          tf::createQuaternionMsgFromYaw(std::atan2(direction.y(), direction.x()));
    }
  }
  ...
  if (!isFinalPathSafeWithDirtyRootEscape(path_optimized, robot_box_size_,
      ...
      !isPathClearanceSafe(path_optimized, "path_optimizer/final")) {
    ROS_WARN(
        "[RRG][OPT] optimized path failed final safety check time=%.4fs",
        (ros::WallTime::now() - start_time).toSec());
    path_optimized.clear();
    return false;
  }
  ...
  ROS_WARN(
      "[RRG][OPT] accepted iterations=%d moves=%d time=%.4fs max_dev=%.3f "
      "clearance=%.3f->%.3f cost=%.3f->%.3f size=%zu",
      planning_params_.path_optimizer_iterations, accepted_moves,
      (ros::WallTime::now() - start_time).toSec(), max_deviation,
      raw_min_clearance, optimized_min_clearance, raw_clearance_cost,
      optimized_clearance_cost, path_optimized.size());
  return true;
}
