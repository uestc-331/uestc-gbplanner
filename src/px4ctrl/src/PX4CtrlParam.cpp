...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
	...
	// ADRC (ESO) parameters (optional)
	nh.param("adrc/w0_xy", adrc.w0_xy, 25.0);
	nh.param("adrc/w0_z", adrc.w0_z, 25.0);
	nh.param("adrc/b0_xy", adrc.b0_xy, 1.0);
	nh.param("adrc/b0_z", adrc.b0_z, 1.0);
	...
	read_essential_param(nh, "msg_timeout/stereo", msg_timeout.stereo);
	read_essential_param(nh, "msg_timeout/stereo", msg_timeout.turnyaw);
	...
	read_essential_param(nh, "ctrl_type", ctrl_type);
	read_essential_param(nh, "use_mocap_ctrl", use_mocap_ctrl);
	...
	read_essential_param(nh, "auto_takeoff_land/land_bias_x", takeoff_land.land_bias_x);
	read_essential_param(nh, "auto_takeoff_land/land_bias_y", takeoff_land.land_bias_y);
