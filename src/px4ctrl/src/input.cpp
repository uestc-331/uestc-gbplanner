...
void RC_Data_t::feed(mavros_msgs::RCInConstPtr pMsg)
{
    ...
    //     enter_hover_mode = true;//第一次进入hover
    // else
    //     enter_hover_mode = false;
    ...
    // // 2
    // if (is_hover_mode)//只有在悬停下才能进cmd，
    // {
    //     if (last_gear < GEAR_SHIFT_VALUE && gear > GEAR_SHIFT_VALUE)
    //         enter_command_mode = true;   //第一次进cmd
...
void Odom_Data_t::mocap_pos_feed(const geometry_msgs::PoseStampedConstPtr &msg)
{
    // ROS_INFO("receive mocap pos");
    ros::Time now = ros::Time::now();
    rcv_stamp = now;
    recv_new_msg = true;
...
void Odom_Data_t::mocap_vel_feed(const geometry_msgs::TwistStampedConstPtr &msg)
{
    // ROS_INFO("receive mocap vel");
    ros::Time now = ros::Time::now();
    rcv_stamp = now;
    recv_new_msg = true;
...
void Imu_Data_t::feed(sensor_msgs::ImuConstPtr pMsg)
{
    ...
    if ((now - last_clear_count_time).toSec() > 1.0)
    {
        if (one_min_count < 45)
        {
            ROS_WARN("IMU frequency seems lower than 45Hz, which is too low!");
        }
        one_min_count = 0;
        last_clear_count_time = now;
    }
