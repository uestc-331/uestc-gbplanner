...
void Rrg::reset() {
  ...
  VoxelStatus voxel_state = map_manager_->getBoxStatus(
      start_check_center, robot_box_size_, true);
  visualization_->visualizeStartCheckBox(start_check_center, robot_box_size_,
                                         voxel_state);
  ROS_INFO(
      "[RRG][START_CHECK_BOX] status=%s center=[%.2f %.2f %.2f] "
      "size=[%.2f %.2f %.2f] center_offset=[%.2f %.2f %.2f]",
      voxelStatusToString(voxel_state), start_check_center.x(),
      start_check_center.y(), start_check_center.z(), robot_box_size_.x(),
      robot_box_size_.y(), robot_box_size_.z(), robot_params_.center_offset.x(),
      robot_params_.center_offset.y(), robot_params_.center_offset.z());
  if (VoxelStatus::kFree != voxel_state) {
    ...
    map_manager_->augmentFreeBox(start_check_center, robot_box_size_);
  }
