...
struct SensorParamsBase {
  ...
  double getFrontierPercentageThreshold() const {
    return frontier_percentage_threshold;
  }
  double getNumVoxelsFullFov() const { return num_voxels_full_fov; }
...
struct PlanningParams {
  ...
  // Oscillation prevention and weak-path filtering.
  double path_history_penalty;
  int path_history_size;
  double path_history_decay_time;
  bool path_reverse_hard_filter;
  double path_reverse_dot_threshold;
  bool wall_clearance_enable;
  double wall_clearance_min;
  double wall_clearance_soft;
  double wall_clearance_penalty;
  double wall_clearance_sample_step;
  bool wall_clearance_reject_final_path;
  bool centerline_bias_enable;
  double centerline_clearance_target;
  double centerline_penalty;
  double centerline_sample_step;
  bool path_smoothness_enable;
  double path_smoothness_penalty;
  bool final_shortcut_enable;
  int final_shortcut_max_skip;
  double final_shortcut_min_savings;
  double final_shortcut_clearance_margin;
  bool path_optimizer_enable;
  int path_optimizer_iterations;
  double path_optimizer_step_size;
  double path_optimizer_smooth_weight;
  double path_optimizer_clearance_weight;
  double path_optimizer_anchor_weight;
  double path_optimizer_max_deviation;
  double path_optimizer_gradient_step;
  double path_optimizer_min_clearance;
  int path_optimizer_fix_start_points;
  bool root_edge_relax_enable;
  double root_edge_relax_distance;
  bool forward_exploration_enable;
  double forward_exploration_min_dot;
  double forward_exploration_soft_dot;
  double forward_exploration_penalty;
  bool forward_completion_only;
  int forward_completion_min_valid_paths;
  bool start_recovery_enable;
  double start_recovery_radius_min;
  double start_recovery_radius_max;
  double start_recovery_radius_step;
  double start_recovery_angle_step_deg;
  bool start_recovery_prefer_forward;
  bool start_recovery_allow_unknown;
  double start_recovery_min_clearance;
  double start_recovery_ignore_start_dist;
  double start_recovery_path_resolution;
  bool dirty_start_edge_tolerance_enable;
  double dirty_start_edge_tolerance_dist;
  bool dirty_root_escape_enable;
  double dirty_root_escape_dist;
  double dirty_root_escape_min_clearance_gain;
  double dirty_root_escape_min_target_clearance;
  double min_gain_threshold;
  double min_path_length;
  double min_unknown_voxels_per_meter;
  ...
  Eigen::Vector3d global_bound_robot_size;
  double global_bound_tolerance;
  bool preplan_recheck_near_robot_relax_enable;
  double preplan_recheck_near_robot_relax_distance;
